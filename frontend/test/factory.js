...
describe("NaegiftEscrowFactory and naegift_escrow", function () {
  ...
  it("상품 사용 완료", async function () {
    ...
    await naegiftEscrowInstance.connect(marketSigner).confirmFulfillment();

    await expect(
      naegiftEscrowInstance.connect(receiverSigner).confirmProductUsed()
  ...
  it("판매대금 정산", async function () {
    ...
    await naegiftEscrow.connect(addr3).confirmProductUsed();
