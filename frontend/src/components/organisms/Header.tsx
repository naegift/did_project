...
const Header: React.FC = () => {
  ...
  const walletConnect = async () => {
    ...
    setApi(mascaApi);
    const currentDID = await api.getDID();
    console.log("currentDID: ", currentDID.data);
    setDid(currentDID.data);
  };

  const createVC = async () => {
    if (!api) return;
    const receiverAddress = "0x76F5728025cad20853726D74d32c54705569e4f5";
    ...
    console.log("VC creation Reponse, vcs in the data: ", res);
    // setVcs(res.data);

    // axios.post('http://localhost:4000/')

    // VC 조회 ()
    let saved = await api.queryCredentials();
    ...
    const document = await api.resolveDID(did);
    console.log(
    ...
    // console.log(vp.data);
    console.log("Verification response: ", vpRes);

    // VC 삭제
    const deleted = await api.deleteCredential(
      saved.data[saved.data.length - 1].metadata.id,
      {
        store: ["snap"],
      }
    );
    ...
    saved = await api.queryCredentials();
    console.log("Queried again to check deletion: ", saved);
  };
  ...
  return (
    <div className="flex flex-row justify-between p-5 border-b">
      ...
      <div className="flex flex-row gap-10">
        ...
        <button onClick={createVC}>Create VC test</button>
