...
const ReceiveBox: React.FC<GiftListData> = ({ receivedItem }) => {
  ...
  const confirm = async () => {
    try {
      setLoading(true);
      const confirmUrl = `${protocol}${process.env.REACT_APP_AWS}/gift/${receivedItem.id}/confirm`;
  ...
  const verify = async () => {
    try {
      ...
      const verifyRes = await axios.patch(
        `${protocol}${process.env.REACT_APP_AWS}/gift/${receivedItem.id}/use`,
        targetVc
      );
  ...
  const receiveGift = async () => {
    try {
      ...
      const response = await axios.patch(
        `${protocol}${process.env.REACT_APP_AWS}/gift/${receivedItem.id}/receive`,
        {
          signature,
          title: message.title,
      ...
      const issueVcUrl = `${protocol}${process.env.REACT_APP_AWS}/gift/${receivedItem.id}/issue`;
