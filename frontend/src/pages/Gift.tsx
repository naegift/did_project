...
const Gift: React.FC = () => {
  ...
  const recevieGiftData = async (_walletAddress: string, page: number) => {
    try {
      const url = `${process.env.REACT_APP_AWS}/gift?receiver=${_walletAddress}&page=${page}&order=${receiveOrder}`;
  ...
  const payGiftData = async (_walletAddress: string, page: number) => {
    try {
      const url = `${process.env.REACT_APP_AWS}/gift?buyer=${_walletAddress}&page=${page}&order=${payOrder}`;
      const response = await axios.get(url);
      // console.log(`response from ${url}`, response);
      // console.log(response.data.gifts);
